      ...
      >
        ...
        {cameraOk && (
          <button
            type="button"
            onClick={() =>
              onCamera && onCamera({ field, label, handler: onChange })
            }
            style={{
              flex: '1 1 48%',
              cursor: 'pointer',
...
export default function StaffStudents() {
  ...
  useEffect(() => {
    ...
    fetchCourses()
    fetchDiscounts()
  }, [fetchStudents, fetchCourses, fetchDiscounts])
  ...
  const handleCourseChange = (courseId) => {
    ...
    setForm((f) => ({
      ...f,
      course: courseId,
      totalFees: c ? String(c.fees || c.defaultFees || '') : '',
      courseDuration: c ? String(c.duration || '') : '',
      numInstallments: 'none',
      initialPayment: '0',
    }))
    setCouponInfo(null)
    if (c) {
      setFinalFees(c.fees || c.defaultFees || 0)
      setAvailableInst(c.installmentOptions || [1, 2, 3, 4, 6, 12])
    }
  }
  ...
  const validateCoupon = async () => {
    ...
    try {
      ...
      showAlert(
        'success',
        `Coupon applied! Full payment of ₹${data.finalFees.toLocaleString('en-IN')} required.`,
      )
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (docs.qualificationDoc)
        fd.append('qualificationDoc', docs.qualificationDoc)
  ...
  const handleAddPayment = async (e) => {
    ...
    if (Number(paymentForm.amount) > selectedStudent.pendingFees)
      return showAlert(
        'error',
        `Cannot exceed pending (₹${selectedStudent.pendingFees.toLocaleString('en-IN')})`,
      )
    setSubmitting(true)
    try {
  ...
  const handleDocumentUpload = async (e) => {
    ...
    if (
      ...
      return showAlert('error', 'Select at least one file')
    setSubmitting(true)
    try {
      const fd = new FormData()
      if (uploadDocModal.studentPhoto)
        fd.append('studentPhoto', uploadDocModal.studentPhoto)
      if (uploadDocModal.qualificationDoc)
        fd.append('qualificationDoc', uploadDocModal.qualificationDoc)
      if (uploadDocModal.aadharCard)
        fd.append('aadharCard', uploadDocModal.aadharCard)
  ...
  const handleEditCropApply = (field, croppedFile) => {
    ...
    const r = new FileReader()
    r.onload = (ev) =>
      setEditPreviews((p) => ({ ...p, [field]: ev.target.result }))
    r.readAsDataURL(croppedFile)
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (editForm.certificateName.trim())
        fd.append('certificateName', editForm.certificateName.trim())
      ...
      if (editForm.couponCode.trim())
        fd.append('couponCode', editForm.couponCode.trim())
      ...
      if (editDocs.studentPhoto)
        fd.append('studentPhoto', editDocs.studentPhoto)
      if (editDocs.qualificationDoc)
        fd.append('qualificationDoc', editDocs.qualificationDoc)
      ...
      if (selectedStudent?._id)
        downloadInvoice(
          selectedStudent._id,
          `${editForm.firstName}_${editForm.lastName}`,
        )
    ...
    <div>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const max = Number(form.totalFees)
                        if (v <= max)
                          setForm({ ...form, initialPayment: e.target.value })
                        else
                          showAlert(
                            'error',
                            `Cannot exceed ₹${max.toLocaleString('en-IN')}`,
                          )
                      }}
                    />
                    <span className="form-hint">
      ...
      {showDocModal && selectedStudent && (
        <div
          className="modal-overlay"
          onClick={(e) =>
            e.target === e.currentTarget && setShowDocModal(false)
          }
        >
          <div className="modal modal-sm">
            <div className="modal-header">
