...
export default function StaffEnquiries() {
  ...
  const counts = {
    all: enquiries.filter((e) => e.status !== 'converted').length,
  ...
  const handleAdmit = async (enquiry) => {
    try {
      await api.put(`/enquiries/${enquiry._id}`, { status: 'converted' })
    } catch (e) {
      console.error('Failed to mark enquiry as converted:', e)
    }
  ...
  const openEdit = (enq) => {
    ...
    setForm({
      ...
      expectedAdmissionDate: enq.expectedAdmissionDate
        ? enq.expectedAdmissionDate.split('T')[0]
        : '',
  ...
  const downloadEnquiry = async (e) => {
    try {
      const response = await api.get(`/enquiries/${e._id}/pdf`, {
        responseType: 'blob',
      })
      ...
      a.href = url
      a.download = `Enquiry_${e.firstName}_${e.lastName}_${e.phoneNumber}.pdf`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
    } catch (err) {
      console.error('Failed to download enquiry PDF', err)
    }
  }
      ...
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">
            {counts.new} new, {counts.contacted} contacted
          </p>
        </div>
        <button
                  ...
                  {paginated.map((e) => (
                    <tr key={e._id}>
                      ...
                      <td data-label="Status">
                        <span
                          className={`badge ${e.status === 'new' ? 'badge-info' : e.status === 'contacted' ? 'badge-warning' : e.status === 'converted' ? 'badge-success' : 'badge-gray'}`}
                        >
                          {e.status === 'new'
                            ? '🆕 New'
                            : e.status === 'contacted'
          ...
          <div className="modal">
            <div className="modal-header">
              <h3 className="modal-title">
                {editing ? '✏️ Edit Enquiry' : '➕ Add Enquiry'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      Interested Course <span className="required">*</span>
                    </label>
                    <select
                      className={`form-select ${errors.interestedCourse ? 'error' : ''}`}
                      value={form.interestedCourse}
                      onChange={(e) =>
                        setForm({ ...form, interestedCourse: e.target.value })
                      }
                    >
                  ...
                  <div className="form-group">
                    ...
                    <input
                      type="date"
                      className={`form-input ${errors.expectedAdmissionDate ? 'error' : ''}`}
                      min={today}
                  ...
                  <div className="form-group">
                    ...
                    <input
                      type="date"
                      className={`form-input ${errors.followUpDate ? 'error' : ''}`}
                      min={today}
                    ...
                    />
                    {errors.followUpDate && (
                      <span className="form-error">{errors.followUpDate}</span>
                    )}
                  </div>
                  <div className="form-group full-width">
