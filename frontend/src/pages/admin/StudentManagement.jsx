  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setCamError('Camera access denied or unavailable. Please allow camera permissions in your browser and try again.');
      }
    })();
      ...
      <div className="modal modal-sm">
        ...
        <div className="modal-body" style={{ textAlign: 'center', padding: '1rem' }}>
          {camError
            ...
            : <>
                <video ref={videoRef} autoPlay playsInline style={{ width: '100%', maxHeight: 300, borderRadius: 8, background: '#000', display: ready ? 'block' : 'none' }} />
                {!ready && <div style={{ padding: '2rem', color: '#6b7280' }}>Starting camera…</div>}
        ...
        <div className="modal-footer">
          ...
          {ready && !camError && <button className="btn btn-primary" onClick={capture}>📸 Capture Photo</button>}
        </div>
      </div>
    </div>
  );
...
export default function StudentManagement() {
  ...
  useEffect(() => { fetchStudents(); fetchCourses(); fetchActiveDiscounts(); }, [fetchStudents, fetchCourses, fetchActiveDiscounts]);
  ...
  useEffect(() => {
    ...
    if (enquiryData) {
      try {
        ...
        setShowModal(true);
      } catch (e) { console.error('Failed to parse enquiry data', e); }
    }
  ...
  useEffect(() => {
    ...
    if (matched) {
      ...
      setAvailableInstallments(matched.installmentOptions || [1, 2, 3, 4, 6, 12]);
    }
  ...
  const handleDiscountSelect = (e) => {
    ...
    setForm(f => ({
      ...
      numInstallments: 'none',
      initialPayment: String(couponInfo ? couponInfo.finalFees : Number(form.totalFees))
    }));
    setCouponInfo(null);
  };
  ...
  const validateCoupon = async () => {
    ...
    try {
      ...
      showAlert('success', `Coupon applied! Full payment of ₹${data.finalFees.toLocaleString('en-IN')} required (no installments allowed with discount).`);
    } catch (err) {
  ...
  const buildInstallments = () => {
    ...
    if (form.numInstallments === 'none') {
      ...
      return [{ amount: total, dueDate: new Date().toISOString().split('T')[0] }];
    }
    ...
    return Array.from({ length: n }, (_, i) => {
      ...
      return { amount: i === n - 1 ? each + remainder : each, dueDate: dueDate.toISOString().split('T')[0] };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (docs.qualificationDoc) formData.append('qualificationDoc', docs.qualificationDoc);
      ...
      setShowModal(false);
      setForm(emptyForm); setCouponInfo(null); setFinalFees(0);
      setDocs(emptyDocs); setPreviews(emptyPreviews);
      fetchStudents();
    } catch (err) {
  ...
  const handleAddPayment = async (e) => {
    ...
    if (Number(paymentForm.amount) > selectedStudent.pendingFees) return showAlert('error', `Payment cannot exceed pending fees (₹${selectedStudent.pendingFees.toLocaleString('en-IN')})`);
    setSubmitting(true);
    try {
      ...
      fetchStudents();
    } catch (err) { showAlert('error', err.response?.data?.message || 'Failed to add payment'); }
    finally { setSubmitting(false); }
  };
  ...
  const handleUploadDocChange = (field, file) => {
    ...
    if (file.type.startsWith('image/')) {
      const reader = new FileReader();
      reader.onload = ev => setUploadDocPreviews(p => ({...p, [field]: ev.target.result}));
      reader.readAsDataURL(file);
    } else {
  ...
  const handleDocumentUpload = async (e) => {
    e.preventDefault();
    const hasFile = uploadDocModal.studentPhoto || uploadDocModal.qualificationDoc || uploadDocModal.aadharCard;
    if (!hasFile) return showAlert('error', 'Please select at least one file');
    setSubmitting(true);
    try {
      const formData = new FormData();
      if (uploadDocModal.studentPhoto) formData.append('studentPhoto', uploadDocModal.studentPhoto);
      if (uploadDocModal.qualificationDoc) formData.append('qualificationDoc', uploadDocModal.qualificationDoc);
      if (uploadDocModal.aadharCard) formData.append('aadharCard', uploadDocModal.aadharCard);
      await api.post(`/students/${selectedStudent._id}/upload-document`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      showAlert('success', 'Documents uploaded successfully!');
      setShowDocumentModal(false);
      setUploadDocModal({ studentPhoto: null, qualificationDoc: null, aadharCard: null });
      setUploadDocPreviews(emptyPreviews);
      fetchStudents();
    } catch (err) { showAlert('error', err.response?.data?.message || 'Failed to upload documents'); }
    finally { setSubmitting(false); }
  };

  const openDocumentUpload = (student) => { setSelectedStudent(student); setUploadDocModal({ studentPhoto: null, qualificationDoc: null, aadharCard: null }); setUploadDocPreviews(emptyPreviews); setShowDocumentModal(true); };
  ...
  const DocThumb = ({ url, label }) => {
    if (!url) return <span style={{ color: 'var(--gray-400)', fontSize: '0.75rem' }}>—</span>;
    return isImage(url)
                  ...
                  {paginated.map(s => (
                    <tr key={s._id}>
                      ...
                      <td className="td-actions" data-label="Actions">
                        ...
                        <button className="btn btn-sm btn-warning" onClick={() => openDocumentUpload(s)}>📄 Docs</button>
              ...
              <div className="modal-body">
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    ...
                    <input className={`form-input ${errors.qualification ? 'error' : ''}`} placeholder="e.g. B.Tech CSE" value={form.qualification} onChange={e => { const val = e.target.value.replace(/[^a-zA-Z0-9 .]/g, ''); setForm({...form, qualification: val}); }} />
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
                      <select className="form-select" value={form.couponCode} onChange={handleDiscountSelect} style={{ flex: 1 }}>
                        ...
                        {activeDiscounts.map(d => (
                          <option key={d._id} value={d.couponCode}>{d.couponCode} — {d.percentage}% off ({d.description})</option>
                        ))}
                      </select>
                    </div>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      readOnly={!!couponInfo}
                      style={couponInfo ? { background: '#f3f4f6', cursor: 'not-allowed', fontWeight: 600 } : {}}
                      onChange={e => {
                        if (couponInfo) return; // locked when discount applied
                        const val = Number(e.target.value);
                        const maxFees = Number(form.totalFees);
                        if (val <= maxFees) setForm({...form, initialPayment: e.target.value});
                        else showAlert('error', `Initial payment cannot exceed total fees (₹${maxFees.toLocaleString('en-IN')})`);
                      }} />
                    <span className="form-hint">
                      {couponInfo
                        ? <span style={{ color: '#b45309', fontWeight: 600 }}>🔒 Full payment locked — discount applied</span>
                  ...
                  <div className="form-group">
                    ...
                    {couponInfo ? (
                      ...
                      <select className="form-select" value={form.numInstallments} onChange={e => {
                          ...
                          const totalFees = couponInfo ? couponInfo.finalFees : Number(form.totalFees);
                          ...
                          setForm(f => ({ ...f, numInstallments: val, initialPayment: autoPayment }));
                        }}>
                        <option value="none">None (Full payment now)</option>
                        <option value="1">1 Installment</option>
                        {availableInstallments.filter(n => n > 1).map(n => <option key={n} value={n}>{n} installments</option>)}
                      </select>
                    )}
                    <span className="form-hint">
                      {couponInfo
      ...
      {showDetailModal && selectedStudent && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowDetailModal(false)}>
          <div className="modal modal-lg">
            ...
            <div className="modal-body">
              <div className="form-grid">
                <div><div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--gray-500)', textTransform: 'uppercase', marginBottom: '2px' }}>Full Name</div><div style={{ fontWeight: 600 }}>{selectedStudent.firstName} {selectedStudent.fatherName} {selectedStudent.lastName}</div></div>
                <div><div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--gray-500)', textTransform: 'uppercase', marginBottom: '2px' }}>Phone</div><div>{selectedStudent.phoneNumber}</div></div>
                <div><div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--gray-500)', textTransform: 'uppercase', marginBottom: '2px' }}>Course</div><div>{selectedStudent.course?.name}</div></div>
                <div><div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--gray-500)', textTransform: 'uppercase', marginBottom: '2px' }}>Qualification</div><div>{selectedStudent.qualification}</div></div>
                <div className="full-width"><div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--gray-500)', textTransform: 'uppercase', marginBottom: '2px' }}>Address</div><div>{selectedStudent.address}</div></div>
              </div>
              ...
              <div style={{ marginTop: '1.25rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
                  {[
                    ...
                    { field: 'qualificationDoc', label: '📜 Qualification Doc' },
                    ...
                    return (
                      <div key={field} style={{ padding: '0.75rem', background: 'var(--gray-50)', borderRadius: 'var(--radius-sm)', border: `1px solid ${url ? 'var(--success)' : 'var(--gray-200)'}`, textAlign: 'center' }}>
                        ...
                        {url ? (
                          isImage(url)
                            ? <a href={url} target="_blank" rel="noopener noreferrer"><img src={url} alt={label} style={{ width: '100%', height: 80, objectFit: 'cover', borderRadius: 4 }} /></a>
                            : <a href={url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '2rem' }}>📄</a>
                        ) : (
                          <div style={{ fontSize: '0.75rem', color: 'var(--gray-400)' }}>Not uploaded</div>
                        )}
                      </div>
                    );
                  })}
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-warning" onClick={() => { setShowDetailModal(false); openDocumentUpload(selectedStudent); }}>📄 Upload Docs</button>
          ...
          <div className="modal modal-sm">
            ...
            <form onSubmit={handleDocumentUpload}>
                  ...
                  <div className="form-group" key={field} style={{ marginBottom: '1rem' }}>
                    ...
                    {existing && (
                      <div style={{ marginBottom: '0.5rem' }}>
                        {isImage(existing)
                          ? <img src={existing} alt={label} style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 4, border: '2px solid var(--success)' }} />
                          : <a href={existing} target="_blank" rel="noopener noreferrer" style={{ fontSize: '1.5rem' }}>📄</a>}
                    ...
                    {uploadDocPreviews[field] && (
                      <div style={{ marginTop: '0.5rem' }}>
                        {uploadDocPreviews[field] === 'pdf'
                          ? <div style={{ fontSize: '0.8rem', color: '#15803d', fontWeight: 600 }}>📄 {uploadDocModal[field]?.name}</div>
                          : <img src={uploadDocPreviews[field]} alt={label} style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 4, border: '2px solid var(--primary)' }} />
                        }
                      </div>
                    )}
                  </div>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={submitting || (!uploadDocModal.studentPhoto && !uploadDocModal.qualificationDoc && !uploadDocModal.aadharCard)}>
