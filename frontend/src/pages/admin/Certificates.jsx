...
function getCroppedImg(imgEl, pixelCrop) {
  ...
  ctx.drawImage(imgEl, pixelCrop.x * scaleX, pixelCrop.y * scaleY, pixelCrop.width * scaleX, pixelCrop.height * scaleY, 0, 0, canvas.width, canvas.height);
  return new Promise(resolve =>
    canvas.toBlob(blob => resolve(new File([blob], `cropped_${Date.now()}.jpg`, { type: 'image/jpeg' })), 'image/jpeg', 0.92)
  );
}
        ...
        <div className="modal-body" style={{ padding: '1rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <ReactCrop crop={crop} onChange={c => setCrop(c)} onComplete={c => setCompletedCrop(c)}
              aspect={isPhoto ? 4 / 5 : undefined} minWidth={50} minHeight={50}>
...
function CameraModal({ label, onCapture, onClose }) {
  ...
  const capture = () => {
    ...
    canvas.toBlob(blob => { if (blob) onCapture(new File([blob], `photo_${Date.now()}.jpg`, { type: 'image/jpeg' })); }, 'image/jpeg', 0.92);
  };
      ...
      <div className="modal modal-sm" style={{ maxHeight: '100vh', display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div className="modal-body" style={{ textAlign: 'center', padding: '1rem', flex: 1, overflow: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {camError ? (
            ...
            <>
              <video ref={videoRef} autoPlay playsInline style={{ width: '100%', maxHeight: 'calc(100vh - 150px)', borderRadius: 8, background: '#000', display: ready ? 'block' : 'none' }} />
...
export default function Certificates() {
  ...
  const openEdit = async (s) => {
    ...
    try {
      ...
      setEditForm({
        ...
        admissionDate:        data.enrollmentDate ? data.enrollmentDate.split('T')[0] : '',
        courseEndDate:        data.courseEndDate  ? data.courseEndDate.split('T')[0]  : '',
        certificateIssuedDate: data.certificateIssuedDate ? data.certificateIssuedDate.split('T')[0] : '',
      });
    } catch {
  ...
  const handleDocDelete = (field) => {
    if (editPreviews[field] && editPreviews[field] !== 'pdf') URL.revokeObjectURL(editPreviews[field]);
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (editForm.admissionDate)         formData.append('admissionDate',         editForm.admissionDate);
      if (editForm.courseEndDate)         formData.append('courseEndDate',         editForm.courseEndDate);
      if (editForm.certificateIssuedDate) formData.append('certificateIssuedDate', editForm.certificateIssuedDate);
      ...
      if (editDocs.aadharCard)       formData.append('aadharCard',       editDocs.aadharCard);
  ...
  const generateCertificate = async (student) => {
    ...
    try {
      ...
      const response = await api.get(`/certificates/generate/${student._id}?grade=${encodeURIComponent(grade)}`, { responseType: 'blob' });
          ...
          : paginated.length === 0 ? (
            ...
            <div className="table-responsive">
              <div className="table-container">
                <table>
                  <thead><tr><th>Student</th><th>Course</th><th>Admission date</th><th>Payment</th><th>Documents</th><th>SR. No</th><th>Status</th><th>Action</th></tr></thead>
                  <tbody>
                    {paginated.map(s => {
  ...
  <div className="td-name">{s.certificateName || `${s.firstName} ${s.fatherName} ${s.lastName}`}</div>
  {s.certificateName && <div className="td-sub">{s.firstName} {s.fatherName} {s.lastName}</div>}
  <div className="td-sub">{s.phoneNumber}</div>
          ...
          <div className="modal modal-lg">
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                <div className="form-group full-width">
                  <label className="form-label">
                    Certificate Name
                    <span style={{ marginLeft: '6px', fontSize: '0.75rem', color: 'var(--gray-400)', fontWeight: 400 }}>(optional — overrides name printed on certificate)</span>
                  </label>
                  <input
                    className="form-input"
                    placeholder="Leave blank to use First + Last name"
                    value={editForm.certificateName}
                    onChange={e => setEditForm({...editForm, certificateName: e.target.value})}
                  />
                </div>
                <div className="form-group">
              ...
              <div style={{ borderTop: '1px solid var(--gray-200)', paddingTop: '1rem', marginTop: '0.5rem' }}>
                <div style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--gray-700)', marginBottom: '0.75rem' }}>📎 Documents</div>
                <div className="form-grid">
                  {[
                    { field: 'studentPhoto',     label: '🖼️ Student Photo',       hint: 'JPG or PNG · Max 1 MB',                    accept: 'image/jpeg,image/jpg,image/png' },
                    { field: 'qualificationDoc', label: '📄 Qualification Doc',   hint: 'JPG, PNG or PDF · Max 1 MB',               accept: '.pdf,.jpg,.jpeg,.png' },
                    { field: 'aadharCard',       label: '🪪 Aadhar Card',         hint: 'Front side of Aadhar card (JPG, PNG) · Max 1 MB', accept: 'image/jpeg,image/jpg,image/png' },
                    ...
                    const existingUrl  = existing ? `${BASE_URL}${existing}?token=${localStorage.getItem('token')}` : null;
                    return (
                      <div className="form-group" key={field}>
                        ...
                        {field === 'studentPhoto' && existingUrl && (
                          <div style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            {photoLoadErr ? (
                              <div style={{ width: 72, height: 72, borderRadius: 8, border: '2px dashed var(--gray-300)', background: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem', flexShrink: 0 }}>🖼️</div>
                            ) : (
                              <img
                                src={existingUrl}
                                alt="Current photo"
                                ...
                                style={{ width: 72, height: 72, objectFit: 'cover', borderRadius: 8, border: '2px solid var(--gray-300)', cursor: 'zoom-in', flexShrink: 0, display: 'block' }}
                              />
                            )}
                            <div>
                              <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--gray-600)', marginBottom: 2 }}>Current photo</div>
                              {existingName && <div style={{ fontSize: '0.72rem', color: 'var(--gray-400)', marginBottom: 4 }}>{existingName}</div>}
                              {!photoLoadErr && (
                                <div style={{ fontSize: '0.72rem', color: 'var(--primary)', cursor: 'pointer', fontWeight: 600 }} onClick={() => setLightboxSrc(existingUrl)}>🔍 Click to view</div>
                              )}
                            </div>
                          </div>
                        )}
                        ...
                        {field !== 'studentPhoto' && existingName && !editPreviews[field] && (
                          <div style={{ marginBottom: '0.4rem', display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.35rem 0.6rem', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 6 }}>
                            <span style={{ fontSize: '1rem' }}>{existing?.match(/\.pdf$/i) ? '📄' : '🖼️'}</span>
                            <span style={{ fontSize: '0.75rem', color: '#15803d', fontWeight: 600, wordBreak: 'break-all' }}>{existingName}</span>
                            <span style={{ fontSize: '0.7rem', color: 'var(--gray-400)', marginLeft: 'auto', whiteSpace: 'nowrap' }}>Uploaded</span>
                          </div>
                        )}

                        {/* File picker + Camera buttons */}
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                          <label style={{ flex: '1 1 45%', cursor: 'pointer', padding: '0.6rem 1rem', border: '1px solid var(--gray-300)', borderRadius: 'var(--radius-sm)', background: '#fff', fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', minHeight: '44px' }}>
                            📁 {editDocs[field] ? 'Change' : 'Choose File'}
                            <input type="file" style={{ display: 'none' }} accept={accept}
                              onChange={e => handleDocChange(field, e.target.files[0])} />
                          </label>
                          <button type="button"
                            onClick={() => handleCameraCapture(field, label)}
                            style={{ flex: '1 1 45%', cursor: 'pointer', padding: '0.6rem 1rem', border: '1px solid var(--primary)', borderRadius: 'var(--radius-sm)', background: 'var(--primary-light,#eff6ff)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', color: 'var(--primary)', fontWeight: 600, minHeight: '44px' }}>
                            📷 Camera
                          </button>
                        </div>
                        <span style={{ fontSize: '0.75rem', color: 'var(--gray-400)', marginTop: '0.25rem', display: 'block' }}>{hint}</span>

                        {/* New file preview */}
                        {editPreviews[field] && (
                          <div style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            {editPreviews[field] === 'pdf' ? (
                              <div style={{ padding: '0.5rem', background: '#dcfce7', borderRadius: 4, display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: '0.8rem' }}>
                                <span>📄</span>
                                <span style={{ color: '#15803d', fontWeight: 600 }}>{editDocs[field]?.name}</span>
                              </div>
                            ) : (
                              <img src={editPreviews[field]} alt={label}
                                onClick={() => setLightboxSrc(editPreviews[field])}
                                style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: 6, border: '2px solid var(--primary)', cursor: 'zoom-in' }} />
                            )}
                            <button type="button" onClick={() => handleDocDelete(field)}
                              style={{ padding: '0.4rem 0.6rem', background: '#fee2e2', border: '1px solid #fecaca', borderRadius: 6, cursor: 'pointer', fontSize: '0.75rem', color: '#dc2626', fontWeight: 600 }}>
                              🗑️ Remove
                            </button>
                          </div>
                        )}
      ...
      {lightboxSrc && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10002, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out' }}
        >
          <img src={lightboxSrc} alt="Preview" style={{ maxWidth: '90vw', maxHeight: '90vh', borderRadius: 8, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }} />
      ...
      {cropState.src && (
        <ImageCropModal
          ...
          onClose={() => { URL.revokeObjectURL(cropState.src); setCropState({ src: null, field: null }); }}
        />
      )}
